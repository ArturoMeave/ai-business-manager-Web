...
export default function TaskModal({
    ...
    if (taskToEdit) {
      setFormData({
        ...
        client: taskToEdit.client
          ? typeof taskToEdit.client === "object"
            ? (taskToEdit.client as any)._id
            : taskToEdit.client
          : "",
  ...
  const getPriorityStyle = (priority: string) => {
    if (priority === "high")
      return "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400";
    if (priority === "medium")
  ...
  const getStatusStyle = (status: string) => {
    ...
    if (status === "in progress")
      return "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-900/30 dark:text-sky-400";
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="absolute inset-0 bg-neutral-900/40 dark:bg-black/60 backdrop-blur-sm transition-colors duration-300 touch-none"
          />
            ...
            <div className="max-h-[80vh] overflow-y-auto no-scrollbar touch-pan-y">
              <AnimatePresence mode="wait">
                {modalMode === "view" ? (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 flex items-center">
                          <Calendar className="w-3 h-3 mr-1" /> Fecha
                        </label>
                        <p className="text-sm font-bold text-neutral-900 dark:text-white">
                          {formData.dueDate || "Sin fecha"}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 flex items-center">
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 flex items-center">
                          <Tag className="w-3 h-3 mr-1" /> Categoría
                        </label>
                        <p className="text-sm font-bold text-neutral-900 dark:text-white">
                          {formData.category}
                        </p>
                    ...
                    <div className="flex items-center gap-3 pt-6 border-t border-neutral-100 dark:border-neutral-800">
                      <div
                        className={`px-4 py-2 rounded-full border text-xs font-black uppercase tracking-wider flex items-center ${getStatusStyle(formData.status)}`}
                      >
                      ...
                      <div
                        className={`px-4 py-2 rounded-full border text-xs font-black uppercase tracking-wider flex items-center ${getPriorityStyle(formData.priority)}`}
                      >
                  ...
                  >
                    <div>
                      <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5 block transition-colors">
                        Título de la Tarea *
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full px-4 py-2.5 bg-neutral-50 dark:bg-[#1a1a1a] border border-neutral-200 dark:border-neutral-700 rounded-xl text-sm text-neutral-900 dark:text-white focus:ring-2 focus:ring-neutral-900 dark:focus:ring-neutral-400 outline-none transition-all"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
                      ...
                      >
                        <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5 flex items-center transition-colors">
                          <Calendar className="w-4 h-4 mr-1" /> Fecha
                        </label>
                        <input
                          type="date"
                      ...
                      >
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2.5 bg-neutral-50 dark:bg-[#1a1a1a] border border-neutral-200 dark:border-neutral-700 rounded-xl text-sm text-neutral-900 dark:text-white focus:ring-2 focus:ring-neutral-900 dark:focus:ring-neutral-400 outline-none transition-all"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-2.5 bg-neutral-50 dark:bg-[#1a1a1a] border border-neutral-200 dark:border-neutral-700 rounded-xl text-sm text-neutral-900 dark:text-white focus:ring-2 focus:ring-neutral-900 dark:focus:ring-neutral-400 outline-none transition-all min-h-[80px] resize-none"
                    ...
                    <div className="pt-6 flex items-center justify-end space-x-3 border-t border-neutral-100 dark:border-neutral-800 transition-colors">
                      ...
                      <button
                        type="submit"
                        disabled={isLoading}
                        className="px-5 py-2.5 text-sm font-medium text-white dark:text-neutral-900 bg-neutral-900 dark:bg-white rounded-xl hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors"
                      >
